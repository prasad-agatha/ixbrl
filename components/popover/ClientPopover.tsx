...
export default function ClientsPopover({ user, setModal, row, onDelete }) {
  const popover = (
          ...
          {["Edit", "Delete"].map((ele: any, id: any) => (
            <ListGroup.Item
              ...
              onClick={async () => {
                document.body.click();
                if (ele === "Edit") {
                  const data = exclude(row, ["apexId", "serviceProvider", "_count", "users"]);
                  data["apexId"] = row.apexId ? "1" : "";
                  data["serviceProviderId"] = row.serviceProvider
                    ? {
                        label: row.serviceProvider.name,
                        value: row.serviceProvider.id,
                        field: row.apexId,
                      }
                    : null;
